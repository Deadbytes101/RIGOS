...
cleanup(){ set +e; mountpoint -q "$work/mnt/state" && umount "$work/mnt/state"; mountpoint -q "$work/mnt/b" && umount "$work/mnt/b"; mountpoint -q "$work/mnt/a" && umount "$work/mnt/a"; mountpoint -q "$work/mnt/efi" && umount "$work/mnt/efi"; losetup -d "$loop" 2>/dev/null; }
trap cleanup EXIT
...
sgdisk --attributes=3:set:60 --attributes=4:set:60 "$loop"
sync; losetup -d "$loop"; trap - EXIT
