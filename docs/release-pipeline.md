...
3. Confirm `git status --porcelain` is empty and run `./scripts/build-rc.sh v0.0.1-rc1`.
4. Preserve the complete `dist/v0.0.1-rc1` directory. Every target must pass `sha256sum -c SHA256SUMS` before execution.
