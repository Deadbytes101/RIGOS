...
#[test]
fn alpha19_build_runs_source_and_exact_image_firstboot_gates() {
    ...
    assert!(version.contains("RIGOS_PRODUCT_VERSION=0.0.4-alpha.19"));
    assert!(version.contains("RIGOS_IMAGE_VERSION=0.0.4-alpha.19"));
    assert!(version.contains("RIGOS_BUILD_ORDINAL=19"));
...
#[test]
fn alpha19_primary_image_verifier_requires_real_grub_theme_and_utility() {
